...
const Cart = () => {
  ...
  const SearchProduct=async()=>{
    try{
      const res = await fetch(`/products/findProducts/${id}`)
  ...
  const postData=async()=>{
    const res = await fetch(`/products/cart/add-to-cart`,{
      method:"post",
      headers:{
  ...
  <div className="container px-5 py-24 mx-auto w-[80%]">
    <div className="lg:flex flex-wrap -m-4">
      <div className="p-4 lg:w-3/5 lg:flex bg-gray-800 bg-opacity-40 rounded-lg">
        ...
        {product?(
          <img src={`http://127.0.0.1:8070/products/${product.picture}`} alt="card_image" className='' />  
        ):("loading..")}
        </div>
      </div>
