...
const Dashboard = () => {
  ...
  const isLogout=async()=>{
    try{
      const res = await fetch(`${process.env.REACT_APP_API}/user/logout`)
      if(res.status === 200){
        await Logout()
        Navigate("/login")
      }
    ...
    const LogUser = async()=>{
      try{
        const res = await fetch(`${process.env.REACT_APP_API}/dashboard`,{method:"GET",headers:{
...
<div>
  <div className="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-white dark:bg-gray-700 text-black dark:text-white">
    ...
    <div className="fixed w-full flex items-center justify-between h-14 text-white z-10">
      ...
      {userData?(<img className="w-7 h-7 md:w-10 md:h-10 mr-2 rounded-md overflow-hidden" src={`https://taptechinc-REACT_APP.herokuapp.com/picture/${user}`} alt='avatar' />):(
